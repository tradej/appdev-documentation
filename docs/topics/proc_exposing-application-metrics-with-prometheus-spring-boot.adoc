...
----
+
By default, {SpringBoot}-based applications expose metrics at the `/prometheus` endpoint, if you have the `spring-boot-actuator` dependency in your `pom.xml` file.
